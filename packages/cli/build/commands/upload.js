...
function getAssetManifest(dirname, assetKey) {
    const manifestPath = path_1.default.join(dirname, assetKey.includes('json') ? assetKey : `${assetKey}.json`);
    return JSON.parse(fs_1.default.readFileSync(manifestPath).toString());
}
/**
 ...
 */
async function initConfig(anchorProgram, walletKeyPair, { totalNFTs, mutable, symbol, retainAuthority, sellerFeeBasisPoints, creators, env, cache, cacheName, }) {
    loglevel_1.default.info('Initializing program config');
    try {
        ...
        loglevel_1.default.info(`Initialized config for a candy machine with publickey: ${config.toBase58()}`);
        (0, cache_1.saveCache)(cacheName, env, cache);
        return config;
    }
    catch (err) {
        loglevel_1.default.error('Error deploying config to Solana network.', err);
        throw err;
    }
}
/**
...
async function upload({ files, cacheName, env, keypair, totalNFTs, storage, retainAuthority, mutable, rpcUrl, ipfsCredentials, awsS3Bucket, arweaveJwk, batchSize, }) {
    ...
    if (dedupedAssetKeys.length) {
        ...
        const { properties: { creators }, seller_fee_basis_points: sellerFeeBasisPoints, symbol, } = getAssetManifest(dirname, '0');
        const config = cache.program.config
            ? new web3_js_1.PublicKey(cache.program.config)
