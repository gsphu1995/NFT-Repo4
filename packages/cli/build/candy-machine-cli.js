...
programCommand('upload')
    ...
    const { number, keypair, env, cacheName, storage, ipfsInfuraProjectId, ipfsInfuraSecret, awsS3Bucket, retainAuthority, mutable, rpcUrl, arweaveJwk, batchSize, } = cmd.opts();
...
programCommand('verify_price')
    ...
    if (isNaN(lamports)) {
        return loglevel_1.default.error(`verify_price requires a --price to be set`);
    }
...
programCommand('create_candy_machine')
    ...
    .option('-r, --rpc-url <string>', 'custom rpc url since this is a heavy command')
    .action(async (directory, cmd) => {
    const { keypair, env, price, cacheName, splToken, splTokenAccount, solTreasuryAccount, rpcUrl, } = cmd.opts();
    ...
    if (solTreasuryAccount) {
        const treasuryAccount = new web3_js_1.PublicKey(solTreasuryAccount);
        const treasuryBalance = await (0, accounts_1.getBalance)(treasuryAccount, env, rpcUrl);
